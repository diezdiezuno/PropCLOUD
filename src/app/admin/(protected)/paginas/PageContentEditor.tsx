...
// El editor de contenido de una página. Se usa en dos lugares: la pantalla
// /admin/paginas/[slug] (con `embedded=false`, muestra su encabezado y botón
// de volver) y embebido en la lista de Páginas por tab (`embedded`, sin
// encabezado). Se maneja solo: carga y guarda su propio `settings`.
export default function PageContentEditor({ slug, embedded = false }: { slug: string; embedded?: boolean }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    // Relee antes de escribir y toca SOLO `settings` de esta página. Así, si la
    // lista de Páginas guardó orden o visibilidad mientras tanto —las dos viven
    // en la misma pantalla ahora—, no se revierte. Cada guardado escribe el
    // array entero, pero cada uno preserva lo que no es suyo.
    const supabase = createClient()
    const { data: fresh } = await supabase.from('tenant_config')
      .select('pages_config').eq('tenant_id', tenantId).single()
    const current = ((fresh?.pages_config as PageConfig[] | null) ?? allPages)

    let updatedPages: PageConfig[] = current.map(p =>
      p.slug === slug ? { ...p, settings } : p
    )
    if (!current.find(p => p.slug === slug) && page) {
      updatedPages = [...updatedPages, { ...page, settings }]
    }
  ...
  return (
    <div>
      {/* Encabezado — solo en la pantalla propia; embebido en la lista sobra. */}
      {!embedded && (
