...
export default function ReclutamientoClient({ positions, intro, submissionWhatsapp }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    await fetch('/api/recruit', {
      ...
      body: JSON.stringify({
        nombre: name, apellido: '', telefono: phone, email,
        zona: '', perfil: position, ocupacion: '', motivacion: message,
        cv_link: '', linkedin: '',
      }),
    }).catch(() => {})
    ...
    <div style={{ paddingTop: 'var(--nav-h,68px)', minHeight: '100vh', background: '#f9f9fb' }}>
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '60px 24px 80px' }}>
        ...
        <p style={{ color: '#666', fontSize: 15, marginBottom: 36, lineHeight: 1.7 }}>
          {intro || 'Completá el formulario y nos pondremos en contacto.'}
        </p>

        <form onSubmit={submit} style={{ background: '#fff', borderRadius: 16, padding: 32, border: '1px solid #ebebeb', display: 'flex', flexDirection: 'column', gap: 16 }}>
